...
def manage_addresses(request):
    ...
    if request.method == "POST":
        if "add_address" in request.POST and address_form.is_valid():
            ...
            if ('editing_address_id' in request.session and
                    request.session['editing_address_id'] == address_id):
                del request.session['editing_address_id']
...
def newsletter_signup(request):
    """Handle newsletter subscription signup."""
...
def toggle_newsletter_subscription(request):
    ...
    try:
        ...
        try:
            ...
            user_profile.newsletter_subscribed = (
                not user_profile.newsletter_subscribed
            )
            user_profile.save()

            # Determine status message
            status = (
                "subscribed to" if user_profile.newsletter_subscribed
                else "unsubscribed from"
            )
            messages.success(
                request,
                f"You have successfully {status} our newsletter."
            )
        except Exception as e:
            messages.warning(
                request,
                "Your subscription preference was updated locally, "
                "but there was an issue with the email service."
            )
...
def add_to_wishlist(request, product_id):
    ...
    if request.method == 'POST':
        ...
        updated_wishlist = Wishlist.objects.filter(
            user=request.user
